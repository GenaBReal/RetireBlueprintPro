...
function writeInputs(data, sheetId) {
  try {
    ...
    if (data.expenses && data.expenses.length) {
      ...
      // Protect formula cells in expense section — clear any bad values
      // Row 62 (index 4) and row 70 (index 12) may be formula rows
      try { inp.getRange('C62').clearContent(); } catch(e){}
