...
function doGet(e) {
  ...
  try {
    ...
    if (action === 'runSweep') {
      ...
      // Kept as a fallback — can exceed the front-end timeout on slow sheets.
      return respond(runSweepBatch(ss, inp, e.parameter));
    }

    if (action === 'runSweepOne') {
      // Single-scenario sweep: runs ONE crash level (idx 0-7) and restores immediately.
      // The Stress Test page calls this 8 times in sequence so rows reveal one at a time.
      return respond(runSweepOne(ss, inp, e.parameter));
    }
...
// ══════════════════════════════════════════════════════════════════
// SWEEP ENGINE
// The 8 standard crash scenarios, a shared single-scenario runner, and
// two entry points:
//   runSweepBatch  — runs all 8 server-side in one call (legacy/fallback;
//                    can exceed the front-end timeout, kept for safety).
//   runSweepOne    — runs exactly ONE scenario by index and restores the
//                    sheet immediately. The front end calls this 8 times in
//                    sequence so each row reveals as it finishes and a single
//                    slow/failed scenario never kills the whole run.
// drag = 7 - yr1 :  yr1=+10 -> drag=-3, yr1=0 -> drag=7, yr1=-30 -> drag=37
// ══════════════════════════════════════════════════════════════════

function _rbpSweepScenarios() {
  return [
...
// Runs ONE scenario against the sheet and returns its result object.
// Does NOT restore — the caller is responsible for restoring `orig`.
function _rbpRunOneSweepScenario(ss, inp, orig, s, inflPct, startYr) {
  // ── Write crash params ──
...
// LEGACY/FALLBACK: all 8 scenarios in one server-side call (one save/restore).
function runSweepBatch(ss, inp, p) {
  ...
  var scenarios = _rbpSweepScenarios();

  try {
    for (var si = 0; si < scenarios.length; si++) {
      results.push(_rbpRunOneSweepScenario(ss, inp, orig, scenarios[si], inflPct, startYr));
    }
    // ── Restore ──
    inp.getRange('B12:B20').setValues(orig);
    SpreadsheetApp.flush();
...
// PER-SCENARIO: run exactly one scenario (by index 0-7) and restore immediately.
function runSweepOne(ss, inp, p) {
  function pnum(v, d){ var n=Number(v); return isNaN(n)?d:n; }
  var inflPct = (p.infl !== undefined && p.infl !== '') ? pnum(p.infl, null) : null;
  var startYr = pnum(p.startYr, 0);
  var idx     = pnum(p.idx, -1);

  var scenarios = _rbpSweepScenarios();
  if (idx < 0 || idx >= scenarios.length) {
    return { ok:false, error:'Bad scenario index: ' + p.idx, idx: idx };
  }

  var orig = inp.getRange('B12:B20').getValues();
  try {
    var res = _rbpRunOneSweepScenario(ss, inp, orig, scenarios[idx], inflPct, startYr);
    res.idx = idx;
    // ── Restore immediately so the customer's sheet is never left crashed ──
    inp.getRange('B12:B20').setValues(orig);
    SpreadsheetApp.flush();
    return res;
  } catch(err) {
    try { inp.getRange('B12:B20').setValues(orig); SpreadsheetApp.flush(); } catch(e2){}
    return { ok:false, error:err.toString(), idx: idx };
  }
}
